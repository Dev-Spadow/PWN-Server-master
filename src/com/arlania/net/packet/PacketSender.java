...
public class PacketSender {
	...
	public void sendBroadcast(String text) {
		PacketBuilder out = new PacketBuilder(235, PacketType.BYTE);
		out.putString(text);
		player.getSession().queueMessage(out);
	}
	...
	public void sendSpinNotification(int winningIndex) {
	...
	public void sendNpcPrayerIcon(int npcId, int iconIndex) {
		PacketBuilder out = new PacketBuilder(194);
		out.putShort(npcId);
		out.put(iconIndex);
		player.getSession().queueMessage(out);
	}

	public PacketSender sendItemData(int slot, int standingAnim, int walkingAnim, int runningAnim) {

		PacketBuilder out = new PacketBuilder(201);
		out.put(slot);
		out.putShort(standingAnim);
		out.putShort(walkingAnim);
		out.putShort(runningAnim);
	...
	public PacketSender sendAnnouncement(String str) {
		PacketBuilder out = new PacketBuilder(30, PacketType.BYTE);
		out.putString(str);
		player.getSession().queueMessage(out);
		return this;
	}
	...
	public void sendClientSettings() {
		...
		// if(player.getPlayerSettings().getOrDefault(PlayerSetting.VIEW_DISTANCE, 25) >
		// 50) {
	...
	public PacketSender sendTabs() {
		...
		sendTabInterface(GameSettings.QUESTS_TAB, 45011); // 26600
		sendTabInterface(GameSettings.ACHIEVEMENT_TAB, 47500);
	...
	public PacketSender closeChatInterface() {
		player.getSession().queueMessage(new PacketBuilder(130));
		return this;
	}
	 ...
	 * PacketBuilder builder = new PacketBuilder(53, PacketType.SHORT);
	 * builder.writeShort(interfaceId); builder.writeShort(items.length); for (Item
	 * item : items) { if (item.getAmount() > 254) { builder.writeByte(255);
	 * builder.writeDoubleInt(item.getAmount()); } else {
	 * builder.writeByte(item.getAmount()); } builder.writeLEShortA(item.getId() +
	 * 1); if (interfaceId == 1203) {
	...
	public void replaceImage(int interfaceId, int imgId) {
		sendMessage(":packet:replaceimg " + interfaceId + " " + imgId);
	}

	public void replacePerkButton(int interfaceId, int imgId, String description) {
		sendMessage(":packet:replaceperkbutton " + interfaceId + " " + imgId + " " + description);
	}
	
	public void setQuestTab(int buttonId) {
		sendMessage(":set_quest_tab:"+buttonId);
	}
	...
	public void sendMouseTrigger(int interfaceIndex, boolean hidden) {
		/*
		 * PacketBuilder out = new PacketBuilder(133); out.writeByte(hidden ? 1 : 0);
		 * out.putShort(interfaceIndex); player.getSession().queueMessage(out);
		 */
	}

	public void sendGroups() {
		PacketBuilder out = new PacketBuilder(98, PacketType.SHORT);
		out.writeByte(Group.groups.size());
		for (int index = 0; index < Group.groups.size(); index++) {
			Group g = Group.groups.get(index);
			out.putString(g.owner);
			out.writeByte(World.getPlayerByName(g.owner) == null ? 0 : 1);
			out.writeByte(g.members.size());
			out.writeByte(g.public_group ? 1 : 0);
			out.putString(g.public_group ? g.uniqueCode : "Hidden");
		}
		player.getSession().queueMessage(out);
	}
	...
	public PacketSender sendRights() {
		...
		out.put(player.getRights().getIcon());
		player.getSession().queueMessage(out);
		return this;
	}
	...
	public PacketSender sendPrivateMessage(long name, PlayerRights rights, byte[] message, int size, byte ironman) {
		PacketBuilder out = new PacketBuilder(196, PacketType.BYTE);
		out.putLong(name);
		out.putInt(player.getRelations().getPrivateMessageId());
		out.put(rights.getIcon());
		out.put(ironman);
