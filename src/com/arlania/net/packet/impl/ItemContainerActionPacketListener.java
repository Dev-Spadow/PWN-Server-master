...
import com.arlania.model.input.impl.EnterAmountGroupStorage;
import com.arlania.model.input.impl.EnterAmountGroupStorageDeposit;
...
public class ItemContainerActionPacketListener implements PacketListener {
	...
	private static void firstAction(Player player, Packet packet) {
		...
		case 15265:
			SharedStorage.withdraw(player, slot, id, 1, false);
			break;
		case 6640:
			SharedStorage.deposit(player, slot, id, 1, true, true);
			break;
		
		case -31528:
			player.getForgingManager().handlePossibleItemClick(item.getId());
			break;

		case -31525:
			player.getForgingManager().forge();
			break;
		...
		case -334:
			...
			player.sendMessage("@blu@The shop buys @red@" + name + " for " + value + " Pwnlite Taxbags");
		...
		case -3327:
			for(UpgradeData data : UpgradeData.values()) {
				if(data.getRequired().getId() == item.getId()) {
					...
					player.getPacketSender().sendString(62203, "Tokens required: @gre@" + bagsRequired);
				}
			} 
			break;
		...
		case Bank.INVENTORY_INTERFACE_ID:
			if (!player.isBanking() || !player.getInventory().contains(item.getId()) || player.getInterfaceId() != 5292)
				return;
			if(item.getId() == 6500) {
				LootingBag.displayLootingBagInterfaceBank(player);
				return;
			}
	...
	private static void secondAction(Player player, Packet packet) {
		...
		case 15265:
			SharedStorage.withdraw(player, slot, id, 5, false);
			break;
		case 6640:
			SharedStorage.deposit(player, slot, id, 5, true, true);
			break;
	...
	private static void thirdAction(Player player, Packet packet) {
		...
		case 15265:
			SharedStorage.withdraw(player, slot, id, 10, false);
			break;
		case 6640:
			SharedStorage.deposit(player, slot, id, 10, true, true);
			break;
		case 32621:
			player.getPlayerOwnedShopManager().handleBuy(slot, id, 1);
			break;
	...
	private static void fourthAction(Player player, Packet packet) {
		...
		case 15265:
    		...
    		int amount = group.storage.getCount(id);
			SharedStorage.withdraw(player, slot, id, amount, false);
			break;
		case 6640:
			SharedStorage.deposit(player, slot, id, player.getInventory().getAmount(id), true, true);
			break;
		case 32621:
			player.setInputHandling(new Input() {
	...
	private static void fifthAction(Player player, Packet packet) {
		...
		case 15265:
			player.setInputHandling(new EnterAmountGroupStorage(id, slot));
			player.getPacketSender().sendEnterAmountPrompt("How many would you like to withdraw?");
			//SharedStorage.deposit(player, slot, id, player.getInventory().getAmount(id), true, true);
			break;
		case 6640:
			player.setInputHandling(new EnterAmountGroupStorageDeposit(id, slot));
	...
	public void handleMessage(Player player, Packet packet) {
		if (player.getConstitution() <= 0)
			return;
		//System.out.println("packet.getOpcode(): " + packet.getOpcode());
