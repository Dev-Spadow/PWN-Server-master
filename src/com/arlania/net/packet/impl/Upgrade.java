...
public class Upgrade {
	...
	public static enum UpgItem {
		...
		PET_MAYO(2760, new Item[] { new Item(11425, 10), new Item(5022, 200000) }, 5, 16455),
	...
	public static void init(Player p, int itemId) {
		...
		if (UpgItem.checkReq(p, itemId)) {
			...
			if (random == 1) {
				...
				// String discordmessage = "[Upgrade] " + p.getUsername() + " has upgraded an "
				// + ItemDefinition.forId(itemId).getName();
				// new DiscordManager(DiscordConstant.UPGRADE_LOG, "Upgrade Log", "" +
				// discordmessage).log1();
				...
				p.getPacketSender().sendMessage("@red@[Failed] @bla@You have failed to Upgrade your @red@"
						+ ItemDefinition.forId(itemId).getName());
				// String discordmessage = "[Failed] " + p.getUsername() + " failed to Upgrade
				// an " + ItemDefinition.forId(itemId).getName();
				// new DiscordManager(DiscordConstant.UPGRADE_LOG, "Upgrade Log", "" +
				// discordmessage).log1();
				// p.getPacketSender().sendMessage("" + random);
				tries++;
