...
public class Trading {
	...
	public void requestTrade(Player player2) {
		...
		if(player.getGameMode() == GameMode.GROUP_IRONMAN) {
			...
			if(!group.members.contains(player2.getUsername())) {
				player.sendMessage("You are an Iron Man. You stand alone.");
				return;
			}
		}
		
		if(player.isMiniPlayer() || player2.isMiniPlayer()) {
			return;
		}
		
		if(player2.getGameMode() == GameMode.GROUP_IRONMAN) {
			Group group = Group.forOwner(player2.groupOwner);
            if(group == null) {
                player.sendMessage("Something went from with your group, please contact a admin.");
                return;
            }
			if(!group.members.contains(player.getUsername())) {
				player.sendMessage(player2.getUsername() + " is an Iron Man. He stands alone.");
				return;
			}
		}
		...
		if (Misc.getMinutesPlayed(player) <= 1) {
			player.sendMessage("You need to have played atleast 1 minute to trade.");
			return;
		}
 
		if (!player.getGambling().gambleFinished || !player2.getGambling().gambleFinished) {
			player.sendMessage("You may not trade right now.");
			return;
		}
	...
	public void openTrade() {
		...
		if (!player.getGambling().gambleFinished) {
			player.sendMessage("You may not trade right now.");
			return;
		}
	...
	public void confirmScreen() {
		...
		player.getPacketSender().sendItemContainer(player.getInventory().refreshItems(), 3322);
		/*
		 * Remove all tabs!
		 */
...
//			TextChannel channel = Main.jda.getTextChannelById(620645393029529611L);
//			channel.sendMessage("`" + player.getUsername() + " Gave the following items in trade to " + player2.getUsername() + "`").queue();
//			channel.sendMessage("```" + convertForLogging(player.getTrading().offeredItems) + "```").queue();
		} catch (Exception ignored) {
		}
	}
 
	private String convertForLogging(CopyOnWriteArrayList<Item> offeredItems) {
		String[] returnedItems = new String[offeredItems.size()];
		for(int i = 0; i < returnedItems.length; i++) {
			returnedItems[i] = offeredItems.get(i).getDefinition().getName();
		}
		return Arrays.toString(returnedItems);
