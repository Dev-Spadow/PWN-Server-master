...
import com.arlania.model.DamageDealer;
...
public class CombatBuilder {
	...
	public void attack(Character target) {
		...
		if (character.isNpc()) {
			if (((NPC) character).getId() == 6143 || ((NPC) character).getId() == 6142
					|| ((NPC) character).getId() == 6144 || ((NPC) character).getId() == 6145) {
				return;
			}
		}
		...
		if (combatTask != null && combatTask.isRunning()) {
			...
			if (character.isPlayer()) {
				...
				if (player.isAutocast()
						|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 11605
	...
	public int getDamage(Player player, boolean clearMap) {
	...
	public void addDamage(Character entity, int amount) {
		...
		if (player.isInRaid())
			player.getCustomRaidParty().getOwner().getCustomRaid().incrementDamage(player.getUsername(), amount);
