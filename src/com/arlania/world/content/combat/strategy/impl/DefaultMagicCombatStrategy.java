...
public class DefaultMagicCombatStrategy implements CombatStrategy {
	...
	public boolean canAttack(Character entity, Character victim) {
		...
		if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 11605) {
			player.setCastSpell(CombatSpells.WATER_STRIKE.getSpell());
		} else if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 11609) {
			player.setCastSpell(CombatSpells.DEATHBLUE.getSpell());
		} else if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 3951) {
			player.setCastSpell(CombatSpells.BATTLESTAFF.getSpell());
		} else if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 5129) {
			player.setCastSpell(CombatSpells.DRAGONLAVA.getSpell());
		} else if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 5179) {
			player.setCastSpell(CombatSpells.HELLFIRE.getSpell());
		} else if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 3911) {
			player.setCastSpell(CombatSpells.PURPLESTAFF.getSpell());
		}
	...
	public CombatContainer attack(Character entity, Character victim) {
		...
		if (entity.isPlayer()) {
			...
			if(player.isAutocast() && player.getAutocastSpell() != null)
				player.setCastSpell(player.getAutocastSpell());
			if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 11605) {
				player.setCastSpell(CombatSpells.SEISMIC.getSpell());
			} else if(player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 11609) {
				player.setCastSpell(CombatSpells.DEATHBLUE.getSpell());
