...
public class DefaultRangedCombatStrategy implements CombatStrategy {
	...
	public CombatContainer attack(Character entity, Character victim) {
		...
		if (!player.isSpecialActivated()) {
			...
			if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 2767)
	...
	private static boolean hasAvas(Player player) {
		for (int i : avaIds)
			if (player.getEquipment().get(Equipment.CAPE_SLOT).getId() == i)
				return true;
		return false;
	}
	...
	public static void decrementAmmo(Player player, Position pos) {
		/*if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 12926 || player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 3082) {
		...
		if (hasAvas(player) && Misc.getRandom(2) <= 1)
		...
		if (!hasAvas(player) && player.getFireAmmo() != 15243)
