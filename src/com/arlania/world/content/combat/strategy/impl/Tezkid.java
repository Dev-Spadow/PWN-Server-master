...
public class Tezkid implements CombatStrategy {
	...
	public boolean customContainerAttack(Character attacker, Character target) {
		...
		if(target.getConstitution() <= 0) {
			nomad.forceChat("Nobody will Destroy Pwnlite!");
			return true;
		}
		if(nomad.isChargingAttack())
			return true;

		if (randomNomad >= 0 && randomNomad <= 25) {
			boolean meleeDistance = Locations.goodDistance(nomad.getPosition(), target.getPosition(), 2);
			boolean heal = !nomad.hasHealed() && nomad.getConstitution() < 100000000;
			if(meleeDistance) {
				if (nomad.getConstitution() > 0 && !heal) {
					...
					nomad.forceChat("Emerald.. Aid me..");
					nomad.setChargingAttack(true);
					TaskManager.submit(new Task(1, nomad, false) {
						int ticks = 0;
						@Override
						public void execute() {
							nomad.setConstitution(nomad.getConstitution() + 25000000);
							ticks++;
							if(ticks >= 5) {
								nomad.forceChat("Emerald, I am in your favor.");
			...
			} else if (randomNomad >= 20 && randomNomad <= 29) {
				nomad.setChargingAttack(true);
				nomad.getMovementQueue().setLockMovement(true);
				TaskManager.submit(new Task(1, nomad, false) {
					int ticks = 0;
					...
					public void execute() {
						...
						if(ticks == 7)
							nomad.forceChat("I will not be DESTROYED!");
						if(ticks == 9 || ticks == 11 || ticks == 13) {
							nomad.forceChat("Die!");
							nomad.performAnimation(new Animation(14221));
							nomad.performGraphic(new Graphic(2283));
						}
				...
				TaskManager.submit(new Task(1, nomad, false) {
					...
					public void execute() {
						...
						if(ticks == 5)
							nomad.forceChat("Emerald, please! Allow me to me channel your power!");
						if(ticks == 10)
							nomad.forceChat("Adventurer, prepare to be blown away!");
						if(ticks == 18)
							nomad.forceChat("I call upon you, Emerald!");
						EmeraldWorldBoss.spawn();
						if(ticks == 20)
							nomad.performAnimation(new Animation(14221));
						if(ticks == 23) 
	...
	private static final Animation anim2 = new Animation(14221);
	private static final Animation anim3 = new Animation(14221);
	private static final Graphic gfx2 = new Graphic(435, GraphicHeight.LOW);
	private static final Graphic gfx3 = new Graphic(2283, GraphicHeight.LOW);
}
