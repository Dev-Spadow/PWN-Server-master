...
public class PlayerKillingAttributes {
	...
	private void handleReward(Player o, boolean targetKilled) {
		...
		player.getLocation() == Location.WILDERNESS) {
			...
			if (player.getInventory().contains(17750) || player.getInventory().getFreeSlots() > 0) {
				player.getInventory().add(17750, rewardAmount);
			} else {
				GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(17750, rewardAmount),
