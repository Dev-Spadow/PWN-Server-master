...
public enum MagicSpells {
	...
	public static boolean handleMagicSpells(Player p, int buttonId) {
		...
		case VENGEANCE:
			...
			if (p.getRights() == PlayerRights.ULTRA_DONATOR || p.getRights() == PlayerRights.SUPPORT) {
			...
			if (p.getRights() == PlayerRights.MYSTIC_DONATOR || p.getRights() == PlayerRights.MODERATOR) {
			...
			if (p.getRights() == PlayerRights.CELESTIAL_DONATOR) {
				if (!p.getLastVengeance().elapsed(10000)) {
					p.getPacketSender().sendMessage("You must wait 10 seconds until you can cast vengance!");
					return true;
				}
