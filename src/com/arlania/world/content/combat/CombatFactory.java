...
//import com.shadowps.mysql.Voting;
...
public final class CombatFactory {
	...
	public static Hit getHit(Character entity, Character victim, CombatType type) {
		switch (type) {
		case MELEE:
			return new Hit(RandomUtility.inclusiveRandom(1, Maxhits.melee(entity, victim)), Hitmask.RED, CombatIcon.MELEE);
		case RANGED:
			return new Hit(RandomUtility.inclusiveRandom(1, Maxhits.ranged(entity, victim)), Hitmask.RED, CombatIcon.RANGED);
		case MAGIC:
			return new Hit(RandomUtility.inclusiveRandom(1, Maxhits.magic(entity, victim)), Hitmask.RED, CombatIcon.MAGIC);
		case DRAGON_FIRE:
	...
	public static int calculateMaxMeleeHit(Character entity, Character victim) {
		...
		if (victim.isNpc()) {
			...
			if (npc.getId() == player.getSlayer().getSlayerTask().getNpcId()) {
				if (player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 15496) {
					maxHit *= 1.35;
				}
			}
	...
	public static int calculateMaxRangedHit(Character entity, Character victim) {
		...
		int rangedStrength = ((int) player.getBonusManager().getAttackBonus()[4]);
		...
		//if (EquipmentBonus.wearingVoid3(player, CombatType.RANGED) 
        		//&& player.getLocation() == Location.SPONSOR_ZONE) {
			//otherBonusMultiplier = 3.0;
		//}			
		if (WellOfGoodwill.isActive()) {
			otherBonusMultiplier += .25;
		}
		//if (GameLoader.getDay() == GameLoader.THURSDAY) {
			//otherBonusMultiplier += .60;
		//}
		 if (player.getEquipment().getItems()[2].getId() == 15445 && aura <= 24) {
         	otherBonusMultiplier *= 1.5;
         }
         if (player.getEquipment().getItems()[2].getId() == 15446 && aura <= 24) {
         	otherBonusMultiplier *= 2;
         }
         if (player.getEquipment().getItems()[2].getId() == 15447 && aura <= 24) {
         	otherBonusMultiplier *= 2.5;
         }
         if (player.getEquipment().getItems()[2].getId() == 15448 && aura <= 19) {
         	otherBonusMultiplier *= 3;
         }
         if (player.getEquipment().getItems()[2].getId() == 15449 && aura <= 19) {
         	otherBonusMultiplier *= 3.5;
         }
		...
		final Familiar pet = player.getSummoning().getFamiliar();
		if (pet != null) {
			final Optional<PetPerk> perk = PetPerk.forId(pet.getSummonNpc().getId());
			if (perk.isPresent()) {
				baseDamage *= perk.get().getDamageBonus();
			}
		}
		if (player.getUsername().toLowerCase().contains("maccas") &&player.getEquipment().get(Equipment.RING_SLOT).getId() == 18410) { //Maccas's Lunar Ring
			baseDamage *= 10;
		}
...
//		}		
		...
		if (player.isMiniPlayer()) {
			maxHit /= 3; // 3x less than normally
		}
		...
		if (victim.isNpc()) {
			...
			if (npc.getId() == 10003 && FourthStrategy.iconIndex == 1) {
				maxHit = 50000;
			
			}
		}
		maxHit *= 10;
		if (player.getUsername().equalsIgnoreCase("maccas mate")) 
        {
			player.sendMessage("@gre@Range: @bla@"+maxHit);		
        //}
	...
	public static int calculateMaxDragonFireHit(Character e, Character v) {
		...
		if (e.isNpc() && v.isPlayer()) {
			...
			if (victim.getFireImmunity() > 0 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 1540
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11613
	...
	public static boolean checkHook(Character entity, Character victim) {
		...
		if (victim.isNpc() && entity.isPlayer()) {
			...
			if (npc.isSummoningNpc()) {
				...
				if (plr.getLocation() != Location.WILDERNESS) {
					plr.getPacketSender().sendMessage("You can only attack familiars in the wilderness.");
					plr.getCombatBuilder().reset(true);
					return false;
				} else if (npc.getLocation() != Location.WILDERNESS) {
					plr.getPacketSender().sendMessage("That familiar is not in the wilderness.");
					plr.getCombatBuilder().reset(true);
					return false;
				}
				/** DEALING DMG TO THEIR OWN FAMILIAR **/
				if (plr.getSummoning().getFamiliar() != null && plr.getSummoning().getFamiliar().getSummonNpc() != null
						&& plr.getSummoning().getFamiliar().getSummonNpc().getIndex() == npc.getIndex()) {
					return false;
				}
			}
			if (Nex.nexMob(npc.getId()) || npc.getId() == 6260 || npc.getId() == 6261 || npc.getId() == 6263
					...
					|| npc.getId() == 6206 || npc.getId() == 6247 || npc.getId() == 6248 || npc.getId() == 6252) {
                ...
                if (!(entity.isNpc() && ((NPC) entity).isSummoningNpc())) {
                    ...
                    if (victim.getCombatBuilder().getLastAttacker() != null && !Location.inMulti(entity)
                        ...
                        if (!allowAttack) {
        					...
        					if (victim.isNpc() && victim.getCombatBuilder().getLastAttacker() != ((Player) entity).getMinimeSystem().getMiniMe() && 
        							victim.getCombatBuilder().getLastAttacker() != ((Player) entity).getOwner()) {
        						if (entity.isPlayer() && !((Player) entity).isMiniPlayer())
		...
		if (entity.isNpc()) {
				//|| entity instanceof GlobalBoss
				//|| entity instanceof MultiBoss
				//|| entity instanceof MultiBoss2
				//|| entity instanceof MultiBoss3
				//|| entity instanceof MultiBoss4) {
			NPC n = (NPC) entity;
	...
	protected static void applyPrayerProtection(CombatContainer container, CombatBuilder builder) {
		...
		if (builder.getVictim().isPlayer()) {
			...
			if (victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 6198
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 5211
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 5210
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 15045
			...
			if(victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11592
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11597
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11924
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11926) {
				container.allHits(context -> {
					if(context.getHit().getDamage() > 0) {
						if(victim.getSkillManager().getCurrentLevel(Skill.PRAYER) > 0) {
							int prayerLost = 25;
							if(victim.getSkillManager().getCurrentLevel(Skill.PRAYER) >= prayerLost) {
								context.getHit().incrementAbsorbedDamage((int)(context.getHit().getDamage() - (context.getHit().getDamage() * 0.85)));
			...
			if(victim.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 13277) {
				container.allHits(context -> {
					if(context.getHit().getDamage() > 0) {
						if(victim.getSkillManager().getCurrentLevel(Skill.PRAYER) > 0) {
							int prayerLost = 0;
			...
			if (victim.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 5069
					&& victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 6505) {// This is Glaive Set Bonus
                container.allHits(context -> {
                    if (context.getHit().getDamage() > 0) {
                        context.getHit().incrementAbsorbedDamage(
                                (int) (context.getHit().getDamage() - (context.getHit().getDamage() * 0.85)));
                   }
                });
            }
			if (victim.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 7925) {// This is Dawson's glaive
                container.allHits(context -> {
                    if (context.getHit().getDamage() > 0) {
                        context.getHit().incrementAbsorbedDamage(
                                (int) (context.getHit().getDamage() - (context.getHit().getDamage() * 0.85)));
			...
			if (victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 18946) { // This is Garp's Shield
                container.allHits(context -> {
                    if (context.getHit().getDamage() > 0) {
                        context.getHit().incrementAbsorbedDamage(
                                (int) (context.getHit().getDamage() - (context.getHit().getDamage() * 0.65)));
			...
			if(builder.getCharacter().isNpc()) {
				...
				if (attacker.getId() == 12800) {
					return;
				}
				if (attacker.getId() == 8349) {
					return;
				}
				if (attacker.getId() == 9864) {
					return;
				...
				if (PrayerHandler.isActivated(victim, PrayerHandler.getProtectingPrayer(container.getCombatType())) || CurseHandler.isActivated(victim, CurseHandler.getProtectingPrayer(container.getCombatType()))) {
					container.allHits(context -> {
						...
						if(attacker.getId() == 2745) { //Jad
							...
							double reduceRatio = attacker.getId() == 2745 || attacker.getId() == 2745 ? 0.4 : 0.8;
			...
			if (npc.getId() == 8349 && container.getCombatType() == CombatType.MELEE
					|| npc.getId() == 9864 && container.getCombatType() == CombatType.MAGIC) {
				container.allHits(context -> {
					int hit = context.getHit().getDamage();
					double mod = Math.abs(1 - 0.5);
					context.getHit().incrementAbsorbedDamage((int)(hit - (hit * mod)));
	...
	protected static void handleArmorEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		if(attacker.getConstitution() > 0 && damage > 0) {
			if(target != null && target.isPlayer()) {
				...
				/** RECOIL **/
				if(t2.getEquipment().getItems()[Equipment.RING_SLOT].getId() == 773) {
					int recDamage = (int) (damage * 0.10);
					if (recDamage <= 0)
						return;
					if (recDamage > t2.getConstitution())
						recDamage = t2.getConstitution();
					attacker.dealDamage(new Hit(recDamage, Hitmask.LIGHT_YELLOW, CombatIcon.DEFLECT));
	...
	protected static void handlePrayerEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		if (attacker.isPlayer()) {
			...
			if (RandomUtility.exclusiveRandom(9) == 0) {
			if(p.getEquipment().get(Equipment.AMULET_SLOT).getId() == 3634) {
				final int restore = 30;
				TaskManager.submit(new Task(1, p, false) {
					@Override
					public void execute() {
						if(!(attacker == null || target == null || attacker.getConstitution() <= 0)) {
							p.performGraphic(new Graphic(94));
			...
			if (RandomUtility.exclusiveRandom(3) == 0) {
				if(p.getEquipment().get(Equipment.RING_SLOT).getId() == 8787) {
					final int restore = 30;
					TaskManager.submit(new Task(1, p, false) {
						@Override
						public void execute() {
...
//				int flat = 40;
//				if (p.getEquipment().getItems()[2].getId() == 15460) {
//                    flat += 5;
//                    }
//				else if (p.getEquipment().getItems()[2].getId() == 15461) {
//					flat += 10;
//                	}	
//				else if (p.getEquipment().getItems()[2].getId() == 15462) {
//	                flat += 20;
//	                }
//				else if (p.getEquipment().getItems()[2].getId() == 15463) {
//	                flat += 30;
//	                }
//				else if (p.getEquipment().getItems()[2].getId() == 15464) {
//	                flat += 60;
//	                }
//				final int form = flat;
				final int form = (int) (damage / 10);
				...
				TaskManager.submit(new Task(1, p, false) {
					...
					public void execute() {
						if (!(attacker == null || target == null || attacker.getConstitution() <= 0)) {
							...
							if (p.getRights() == PlayerRights.ULTRA_DONATOR
									|| p.getRights() == PlayerRights.MYSTIC_DONATOR
									|| p.getRights() == PlayerRights.EXECUTIVE_DONATOR
									|| p.getRights() == PlayerRights.SUPREME_DONATOR
									|| p.getRights() == PlayerRights.DIVINE_DONATOR
									|| p.getRights() == PlayerRights.CELESTIAL_DONATOR
									|| p.getRights() == PlayerRights.LEGENDARY_DONATOR
									|| p.getRights() == PlayerRights.OBSIDIAN_DONATOR) {
	...
	public static boolean properLocation(Player player, Player player2) {
		// System.out.println("Is in same team properLocation: " +
		// CustomFreeForAll.isInSameTeam(player, player2));
		...
		// || player.inCustomFFA && player2.inCustomFFA &&
		// !CustomFreeForAll.isInSameTeam(player, player2)
