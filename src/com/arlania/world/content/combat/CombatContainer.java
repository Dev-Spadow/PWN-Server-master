...
public class CombatContainer {
    ...
    private final CombatHit[] prepareHits(int hitAmount) {
        ...
        if (attacker.isPlayer()) {
            if (((Player) attacker).getEquipment()
                .containsAny(20427, 2543, 3311, 5171, 3904, 3077, 6930, 3088, 12426, 5171, 3279, 1666, 3275, 6483, 8474,19727,14922,14921,14923, 13995, 8664,15010,5115, 13201, 4796, 7682, 19154,20927,20943,18887,20934,13207, 5150, 6197,12238, 2760, 932, 16555,5267, 3651, 13024,5263, 14436, 5271, 5272,16470, 5264,14027, 5219, 16450, 13265, 5231, 5214, 18493, 13289,19906,13094, 1413,16524, 18911,8654, 18912,8655, 14581,8656, 5089, 3641, 933, 5130, 5173, 5134, 15656, 3291, 5273, 5274, 5069, 6934, 6498, 17845, 5217, 5216, 7035, 8817, 5116, 17379, 14550, 14910, 16459, 16495, 16468, 16463,16435,17853,16436,14027,13388,8042,14028,14287,14029,16475,16476,16478,8488,8031,15008,15009,11952,15010,3920,8705,12185,8040,20917,4741, 5129, 19620,19618, 19163, 5249, 18931, 8001, 18866,
                    6499,8041,13389,5265, 8818, 3971, 3276,5131, 14535,80,10479, 7764, 3290,16528,3274,14028,16436,16476, 10905,5175, 5195, 5158, 3987, 3991, 5132, 5125, 5081, 5133, 3990, 17926, 17929, 923, 16518,19720, 18957, 15653)) {
                hitAmount = 4; //Client doesn't support more than 4
            }
        }

        if (attacker.isPlayer()) {
            if (((Player) attacker).getEquipment().containsAny(5130, 3928, 3941, 17849,16540,3911,16525,16523)) {
                hitAmount = 2; //Client doesn't support more than 4
            }
        }

        if (attacker.isPlayer()) {
            if (((Player) attacker).getEquipment().containsAny(896)) {
                hitAmount = 1; //Client doesn't support more than 4
            }
        }
        if (attacker.isPlayer()) {
            if (((Player) attacker).getEquipment()
                .containsAny(19468, 11605, 14559, 18683, 14018, 6320,3276, 18663, 18985, 9492, 19123, 4059,16526)) {
                hitAmount = 1; //Client doesn't support more than 4
            }
        }
        ...
        if (attacker.isPlayer() && ((Player) attacker).isSpecialActivated()) {
            if (((Player) attacker).getCombatSpecial() == CombatSpecial.DRAGON_CLAWS && hitAmount == 4) {
                ...
                int third = first <= 0 && second > 0 ? (int) (second / 2) : first <= 0 && second <= 0 ? array[2]
                    .getHit()
                    .getDamage() : Misc.getRandom(second);
                int fourth = first <= 0 && second <= 0 && third <= 0 ? (int) array[3].getHit()
                    .getDamage() + Misc.getRandom(7) : first <= 0 && second <= 0 ? array[3].getHit()
                    .getDamage() : third;
    ...
    public final void dealDamage() {
        if (hits.length == 1) {
            ...
            victim.dealQuadrupleDamage(hits[0].getHit(), hits[1].getHit(), hits[2].getHit(), hits[3]
                .getHit());
        }
    }
