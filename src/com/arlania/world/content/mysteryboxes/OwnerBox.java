	...
	public void process() {
		...
		if (player.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if (randomYoda == 0) {
				...
				World.sendMessage("<shad=bf0000>[Owner Box]</shad>@bla@: " +player.getUsername().toString() + 
		            " has just received a <col=FFFF64><shad=ebf217> " + ItemDefinition.forId(donor[rewardGrade3][rewardPos]).getName() + "");
				...
				if (randomDouble == 0) {
					...
					player.setAnimation(new Animation(6382));
					player.setGraphic(new Graphic(127));
					World.sendMessage("<shad=0>@bla@[@or2@Owner box@bla@] [@blu@Uncommon@bla@] @or2@" + player.getUsername() + "@bla@ Has just received a @or2@ " + ItemDefinition.forId(donor[rewardGrade3][rewardPos]).getName() + " @bla@!");
					double randomDouble = Misc.random(125);
				...
				player.setAnimation(new Animation(6382));
				player.setGraphic(new Graphic(127));
				World.sendMessage("<shad=0>@bla@[@or2@Owner box@bla@] [@gre@common@bla@] @or2@" + player.getUsername() + "@bla@ Has just received a @or2@ " + ItemDefinition.forId(donor[rewardGrade3][rewardPos]).getName() + " @bla@!");
				double randomDouble = Misc.random(125);
				...
				if (randomDouble == 0) {
					...
					player.getInventory().delete(10168, 1);
