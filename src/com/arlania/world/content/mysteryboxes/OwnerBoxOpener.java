...
public class OwnerBoxOpener {

	private Player player;

	public OwnerBoxOpener(Player player) {
		this.player = player;
	}
	...
	public void openAll(int boxId) {
		...
		for (int i = 0; i < amount; i++) {
			...
			if (chance > 99) { //90% of 100% = 10%
				reward = rare[RandomUtility.exclusiveRandom(0, rare.length)];
			} else if (chance > 78) { //65% of 100% = 35% (remainder)
				reward = uncommon[RandomUtility.exclusiveRandom(0, uncommon.length)];
		...
		player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.OPEN_10_BOXES, amount);
		player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.OPEN_25_MYSTERY_BOXES, amount);
		player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.OPEN_250_MYSTERY_BOXES, amount);
		player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.OPEN_1000_MYSTERY_BOXES, amount);
