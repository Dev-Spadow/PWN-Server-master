...
public class StarterZone {
	...
	StarterZoneData currentData = StarterZoneData.BLOOD_REAPER;
	...
	public void handleKilledNPC(NPC npc) {
		...
		if (data != currentData) {
			player.sendMessage("You are not killing the right npc! You should be killing "
					+ NpcDefinition.forId(currentData.npcId).getName());
			return;
		}
		...
		if (newKC == data.killsRequired) {
			...
			if(currentData == StarterZoneData.SHADOWLORD) {
				zoneInterface.close();
				return;
			}
