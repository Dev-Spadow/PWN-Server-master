...
public class SkillManager {
    ...
    public SkillManager addExperience(Skill skill, int experience) {
        ...
        if(WellOfGoodwillHandler.isDoubleXpActive()) {
        	experience *= 1.5;
        }
        ...
        if (this.skills.experience[skill.ordinal()] >= MAX_EXPERIENCE) {
			...
			player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.REACH_MAX_EXP_IN_A_SKILL, 1);
        }
        /*
        ...
        if (newLevel > startingLevel) {
            ...
            if (skills.maxLevel[skill.ordinal()] == getMaxAchievingLevel(skill)) {
                ...
                for (Skill skill3 : Skill.values()) {
                    if (skills.maxLevel[skill3.ordinal()] == getMaxAchievingLevel(skill3)) {
                        i2++;
                    }
                }
                if (i2 == Skill.values().length) {
                    player.getPacketSender().sendMessage("Well done! You've achieved the highest possible level in all skills!");
                    World.sendMessage("@red@[Player News] @bla@" + player.getUsername() + " has just achieved level 150 in every skill!");
                    player.getPacketSender().sendMessage("@blu@Enjoy your max cape!");
                    if (player.getInventory().getFreeSlots() >= 1) {
                        player.getInventory().add(14019, 1);
                    } else {
                        player.getBank(player.getCurrentBankTab()).add(14019, 1);
                        player.getPacketSender().sendMessage("@red@Your cape was sent to your bank!");
                    }
                }
            } else {
    ...
    private static final int MAX_EXPERIENCE = 2033749559;
	...
	public void setCurrentLevel(int i, int j, int k, int l) {
