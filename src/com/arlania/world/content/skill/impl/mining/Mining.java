			...
			if (pickaxe > 0) {
				if (miningLevel >= o.getLevelReq()) {
					...
					if (miningLevel >= p.getReq()) {
						...
						if(Misc.getRandom(5000) == 3) {
							if (player.getGameMode() == GameMode.IRONMAN) {	
							if (player.getGameMode() == GameMode.HARDCORE_IRONMAN) {	
							player.getInventory().add(2795, 1);
							World.sendMessage("@blu@<img=10>[SKILLING] "+player.getUsername()+" has received the Loot box tier 3!");
							player.getPacketSender().sendMessage("@red@You have received a Loot box tier 3!");
						}
					}
				}
						if(Misc.getRandom(3000) == 3) {
							if (player.getGameMode() == GameMode.IRONMAN) {	
							if (player.getGameMode() == GameMode.HARDCORE_IRONMAN) {	
							player.getInventory().add(3578, 1);
							World.sendMessage("@blu@<img=10>[SKILLING] "+player.getUsername()+" has received the Loot box tier 2!");
							player.getPacketSender().sendMessage("@red@You have received a Loot box tier 2!");
						}
					}
				}	
						if(Misc.getRandom(1000) == 3) {
							...
							World.sendMessage("@blu@<img=10>[SKILLING] "+player.getUsername()+" has received the Loot box tier 1!");
							player.getPacketSender().sendMessage("@red@You have received a Loot box tier 1!");
						}
					}
				}	
							...
							public void execute() {
								...
								if (cycle == reqCycle) {
									...
									if (o.getItemId() != -1) {
                                        ...
                                        DailyTasks.INSTANCE.updateTaskProgress(TaskType.MINING, player, o.getItemId(), doubleLoot ? 2 : 1);
									}
									if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 15259) {
