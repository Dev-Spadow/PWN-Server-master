	...
	public static void fletchBow(final Player player, final int product, final int amountToMake) {
		...
		player.setCurrentTask(new Task(2, player, true) {
			...
			public void execute() {
                ...
                DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, product, shafts ? 15 : 1);
                DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, log, 1);
			...
			if (log == g.unStrung()) {
				...
				player.setCurrentTask(new Task(2, player, false) {
					...
					public void execute() {
                        ...
                        DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, g.Strung(), 1);
                        DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, log, 1);
		...
		if (arr != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= arr.getLevelReq()) {
				if (player.getInventory().getAmount(arr.getItem1()) >= 15 && player.getInventory().getAmount(arr.getItem2()) >= 15) {
                    ...
                    DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, arr.getItem1(), 15);
                    DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, arr.getItem2(), 15);
                    DailyTasks.INSTANCE.updateTaskProgress(TaskType.FLETCHING, player, arr.getOutcome(), 15);
