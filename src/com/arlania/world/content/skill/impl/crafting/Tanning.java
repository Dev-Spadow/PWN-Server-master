				...
				if (coins >= price) {
					if (player.getInventory().contains(hide)) {
						...
						if(usePouch) {
                            ...
                            DailyTasks.INSTANCE.updateTaskProgress(TaskType.SHOPPING, player, 995, price);
							player.getPacketSender().sendString(8135, ""+player.getMoneyInPouch()); //Update the money pouch
						} else {
                            DailyTasks.INSTANCE.updateTaskProgress(TaskType.SHOPPING, player, 995, price);
                            player.getInventory().delete(995, price);
                        }
                        DailyTasks.INSTANCE.updateTaskProgress(TaskType.CRAFTING, player, leather, amount);
