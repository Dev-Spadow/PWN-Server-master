...
public class Herblore {
	...
	public static boolean cleanHerb(final Player player, final int herbId) {
		...
		if(player.getInventory().contains(herb.getGrimyHerb())) {
            ...
            DailyTasks.INSTANCE.updateTaskProgress(TaskType.HERBLORE, player, herb.getCleanHerb(), 1);
			return true;
		}
		return false;
	}
		...
		if (player.getInventory().contains(VIAL) && player.getInventory().contains(unf.getHerbNeeded())) {
			...
			TaskManager.submit(new Task(1, player, false) {
				public void execute() {
					...
					player.getPacketSender().sendMessage("@red@You've just received " + 10 + " skilling points.");
					player.getPointsHandler().incrementSkillPoints(10);
                    DailyTasks.INSTANCE.updateTaskProgress(TaskType.HERBLORE, player, unf.getUnfPotion(), 1);
					this.stop();
				}
			});
			return true;
		...
		if(player.getInventory().contains(pot.getUnfinishedPotion()) && player.getInventory().contains(pot.getItemNeeded())) {
			...
			TaskManager.submit(new Task(1, player, false) {
				public void execute() {
					...
					player.getPointsHandler().setSkillPoints(100, true);
					player.getInventory().add(10835, 10);
                    DailyTasks.INSTANCE.updateTaskProgress(TaskType.HERBLORE, player, pot.getFinishedPotion(), 1);
