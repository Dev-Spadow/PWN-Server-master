...
public enum ObstacleData {
	...
	PIPE_2(154, true, false) {
		...
		public void cross(final Player player) {
			...
			player.getInventory().add(10835, 50);
			player.getPointsHandler().setSkillPoints(25, true);
			player.getPacketSender().sendMessage("You attempt to go through the pipe..");
			TaskManager.submit(new Task(1, player, false) {
				int tick = 0;
	...
	RAMP(1948, false, false) {
		...
		public void cross(final Player player) {
			...
			player.getInventory().add(10835, 100);
			player.getPointsHandler().setSkillPoints(75, true);
	...
	STEPPING_STONES(9326, true, true) {
		...
		public void cross(final Player player) {
			...
			if (player.getWildernessLevel() == 0) {
				if (player.getPosition().getY() != 3960) {
					if (player.getPosition().getX() <= 2996) {
						player.getPosition().setY(3960);
					}
				} else if (player.getPosition().getY() != 3960) {
					if (player.getPosition().getX() >= 2996) {
						player.getPosition().setY(3960);
					}
				}
			...
			TaskManager.submit(new Task(2, player, false) {
				...
				public void stop() {
					...
					if (Agility.passedAllObstacles(player)) {
						...
						player.getPointsHandler().setSkillPoints(25, true);
						player.getInventory().add(10835, 75);
