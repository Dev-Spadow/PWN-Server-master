	...
	public static void start(final Player p) {
		...
		for(Player member : party.getPlayers()) {
			if(member != null) {
				...
				member.getInventory().add(17750, amt);
	...
	public static void handleNpcDeath(Player p, NPC n) {
		if(n.getPosition().getZ() == p.getPosition().getZ()) {
			...
			if(random >= 20 && random <= 45 || boss) {
				...
				GroundItemManager.spawnGroundItem(p, new GroundItem(new Item(17750, amt), n.getPosition().copy(), "Dungeoneering", false, -1, false, -1));
