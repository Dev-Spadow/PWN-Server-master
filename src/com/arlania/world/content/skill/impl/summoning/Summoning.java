...
public class Summoning {
	...
	public boolean unsummon(boolean full, boolean dropItems) {
		if(dropItems) {
			if(bob != null) {
            ...
            if (full) {
                if (player.getInventory().getFreeSlots() < 1) {
                    if (!player.getBank(0).isFull()) {
                        final BossPet lol = BossPet.forSpawnId(spawnId);
                        if (lol == null) {
                            System.out.println("wtf?!!");
                        }

                        player.getBank(0).add(BossPet.forSpawnId(spawnId).getItemId(), 1);
                        player.sendMessage("Your pet was added to your bank.");
                    } else {
                        player.sendMessage("You do not have enough free inventory space to do that!");
                        return false;
                    }
                } else {
		...
		TaskManager.submit(new Task(1, n, true) {
			...
			protected void execute() {
				...
				if(!familiar.isPet() && n.getDefinition().isAttackable() && (underAttack || attacking)) {
					...
					//should make pets attack npcs
					if(Location.inMulti(player)) {
						
						n.setPetCombat(true);
						
						n.getCombatBuilder().attack(attacking ? player.getCombatBuilder().getNpc() : player.getCombatBuilder().getLastAttacker());
						
						n.setEntityInteraction(n.getCombatBuilder().getNpc());
					}
