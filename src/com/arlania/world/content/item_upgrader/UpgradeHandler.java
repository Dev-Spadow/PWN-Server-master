...
public class UpgradeHandler {
	...
	public void init() {
		...
		player.getPA().sendFrame126("Item enchantment ", 47255);
	}
		...
		for (UpgradeData data : data) {
			if (data.getType() == player.getUpgradeType()) {
				if (buttonId == data.getClickId()) {
					...
					player.getPA().sendFrame126("Success rate: " + data.getSuccessRate() + "%", 47262);
		...
		if (player.getCurrentUpgrade() != null) {
			for (int i = 0; i < player.getCurrentUpgrade().getIngredients().length; i++) {
				if (player.getInventory().getAmount(player.getCurrentUpgrade().getIngredients()[i]
					...
					player.sendMessage("You don't have the required ingredients to create" + "the "
			...
			if (randomInt <= player.getCurrentUpgrade().getSuccessRate()) {
				...
				player.sendMessage("Congratulations, you successfully created the"
						+ ItemDefinition.forId(player.getCurrentUpgrade().getResultItem()).getName() + ".");
				...
				player.sendMessage("Aww, unfortunately you failed to create the"
						+ ItemDefinition.forId(player.getCurrentUpgrade().getResultItem()).getName()
						+ ". Better luck next time.");
			}
			// }
		} else {
			player.sendMessage("You silly! Please select an item first.");
		}
	}
}
