...
import com.arlania.world.content.slayer.slayer1;
	...
	public static void handle(Player player, int id) {
		...
		if (id == FIRST_OPTION_OF_FIVE) {
			...
			case 2501: // RAID 1 SELECTION
				player.getRaidParty().setRaid(new Raid1(null, "Raid 1", player.getRaidParty()));
			...
			case 9:
				...
				break;
			case 2503:
				TeleportHandler.teleportPlayer(player, new Position(3148, 9265),
						player.getSpellbook().getTeleportType());
				break;
		...
		//	case 68:
			...
			//case 2501: // RAID 2 SELECTION
				//player.getRaidParty().setRaid(new Raid2(null, "Raid 2", player.getRaidParty()));
				//player.getPacketSender().closeAllWindows();
				//break;
			case 1:
			...
			case 1111: //elder boss 1 option
			if (player.getInventory().getAmount(10835)  < 10000000) {
				
				player.getPacketSender()
			.sendMessage("@blu@You need at least 10m Pwnlite Taxbags"
			+ "to fight the great one ");
			return;
			}
			TeleportHandler.teleportPlayer(player, new Position(2802, 9148),
			player.getSpellbook().getTeleportType());
			player.getInventory().delete(10835, 10000000);
			break;
			
			
			case 102:// red stone take
				if (player.getInventory().getAmount(15286)  < 15000) {
					
					player.getPacketSender()
				.sendMessage("@blu@You need at least 10k @red@Stone clippings"
				+ "@red@To get the stone");
				return;
				}
				player.getInventory().delete(15286, 15000);
				player.getInventory().add(3357, 1);
				break;
			
			case 1020:// yellow stone take
				if (player.getInventory().getAmount(3357)  < 1) {
					
					player.getPacketSender()
				.sendMessage("@blu@You need at least 1 @red@Red stone"
				+ "@red@To get the aura");
				return;
				}
				player.getInventory().delete(3357, 1);
				player.getInventory().add(3309, 1);
				break;	
			...
			//case 2501: // RAID 3 SELECTION
				//player.getRaidParty().setRaid(new Raid3(null, "Raid 3", player.getRaidParty()));
				//player.getPacketSender().closeAllWindows();
				//break;
			case 1:
			...
			case 13:
				...
				player.getPacketSender().sendMessage("Speak To Emerald to obtain this rank..");
			...
			case 2500: // CREATE RAID GROUP
				DialogueManager.start(player, Arianwyn.getDialogue(player, 1));
				break;
			case 2506: // CREATE RAID GROUP
				DialogueManager.start(player, PennyWiseDialouge.getDialogue(player, 1));
				break;
			...
			case 743:
				if ((GameSettings.TRIPLE_EXP == true) || (GameSettings.DOUBLE_EXP == true)) {
					...
					player.getPacketSender().sendInterfaceRemoval();
				} else if (player.getInventory().contains(10835, 30000)) {
					player.getInventory().delete(10835, 30000);
					player.incrementCustomWellDonated(1);
					GameSettings.TRIPLE_EXP = true;
					World.sendMessage("<img=10>@blu@" + player.getUsername().toString()
							+ " @bla@Has just donated @red@30000@bla@ for @red@3h Triple Exp (serverwide)");
					Timer t = new Timer();
					...
					player.sendMessage("@blu@You don't have 30000 Pwnlite Taxbags in ur inv :(");
					player.getPacketSender().sendInterfaceRemoval();
				}
				break;
			/*
			 ...
			 * + " @red@Has just donated 200 Pwnlite Taxbags for Triple Exp");
			 * player.getPacketSender().sendInterfaceRemoval(); }
			 * 
			 * } else { player.sendMessage("@blu@You don't have 200 Pwnlite Taxbags in ur inv :(");
			 * player.getPacketSender().sendInterfaceRemoval(); break; }
			 */
			case 5:
				player.getPacketSender().sendInterfaceRemoval();
			...
			case 99: //rmw glaive
				 if (player.getInventory().getAmount(3706)  < 250) {
						
						player.getPacketSender()
					.sendMessage("@blu@You need at least 250 @red@Starter tokens"
					+ "@red@To get the glaive");
					return;
					}
					player.getInventory().delete(3706, 250);
					player.getInventory().add(3278, 1);
					break;
			
			
			case 102:// red stone take
				if (player.getInventory().getAmount(15286)  < 18000) {
				...
				player.getInventory().delete(15286, 18000);
				player.getInventory().add(3347, 1);
				break;
			...
			case 333:
				player.getBloodSlayer().assignBloodSlayerTask();
				break;
				

				
			case 334:
				DialogueManager.start(player, BloodSlayerDialogues.findAssignment(player));
				break;
			...
			case 41:
				...
				break;
			case 45:
			    if(player.getGameMode().equals(GameMode.GROUP_IRONMAN))
                    GamemodeSelecter.leaveGroup(player);
			...
			case 2500: // this one is for that option
				if (player.getInventory().contains(8987, 1)) {
					player.sendMessage("You already have a torch, Go find Gorgie!");
					player.getPacketSender().sendInterfaceRemoval();
					return;
				}
			player.getInventory().add(8987, 1);
			player.getPacketSender().sendInterfaceRemoval();
				break;
			case 2506:
				player.performAnimation(new Animation(6387));
				player.sendMessage("You climb down the old dirty manhole into the sewers.");
				player.moveTo(new Position(3164, 9893));
					break;
			case 101:
				if (player.getClickDelay().elapsed(4500)) {
				...
				break;
			case 99: //staff of grace
				 if (player.getInventory().getAmount(3706)  < 250) {
						
						player.getPacketSender()
					.sendMessage("@blu@You need at least 250 @red@Starter tokens"
					+ "@red@ To claim the staff");
					return;
					}
					player.getInventory().delete(3706, 250);
					player.getInventory().add(3291, 1);
					break;
			
			
			case 1111: //elder boss 2 option
				if (player.getInventory().getAmount(10835)  < 10000000) {
			...
			case 102:// blue stone take
				...
				player.getInventory().delete(15286, 10000);
				player.getInventory().add(3351, 1);
				break;
			case 1020:// yellow stone take
				if (player.getInventory().getAmount(3351)  < 1) {
					
					player.getPacketSender()
				.sendMessage("@blu@You need at least 1 @red@Blue Stone"
				+ "@red@To get the aura");
				return;
				}
				player.getInventory().delete(3351, 1);
				player.getInventory().add(3310, 1);
				break;
			case 743:
				if (GameSettings.DOUBLE_BOSSPOINTS == true) {
					player.sendMessage("@blu@The Custom Well Of Goodwill is currently full");
					player.getPacketSender().sendInterfaceRemoval();
				} else if (player.getInventory().contains(10835, 500000)) {
					player.getInventory().delete(10835, 500000);
					player.incrementCustomWellDonated(1);
					GameSettings.DOUBLE_BOSSPOINTS = true;
					World.sendMessage("<img=10> @blu@" + player.getUsername().toString()
							+ " @bla@Has just donated @red@500k bags @bla@for @red@3h Double MOB Points (serverwide)");
					Timer t = new Timer();
					t.schedule(new TimerTask() {
						@Override
						public void run() {
							GameSettings.DOUBLE_BOSSPOINTS = false;
							World.sendMessage("<img=10>@blu@ Double MOB Points has ended");
						}

					}, 10800000);
					player.getPacketSender().sendInterfaceRemoval();
				} else {
					player.sendMessage("@blu@You don't have 500k Pwnlite Taxbags in ur inv :(");
					player.getPacketSender().sendInterfaceRemoval();
				}
				break;
			case 5:
			...
			case 13:
				...
				player.getPacketSender().sendMessage("Speak To Pwnlite to obtain this rank..");
				// DialogueManager.start(player, 122);
				// player.setDialogueActionId(76);
				break;
			case 14:
			...
			switch (player.getDialogueActionId()) {
			case 2500:
				if ((player.getInventory().getAmount(14986)  < 1) ||(player.getInventory().getAmount(14983)  < 1)
						||(player.getInventory().getAmount(14984)  < 1)
						||(player.getInventory().getAmount(14985))  < 1) {
					player.sendMessage("@red@You don't have all the items to bring back Gorgie");
					player.getPacketSender().sendInterfaceRemoval();
				return;
				}
					if (player.getInventory().containsAll(14986, 14983, 14984, 14985)) {
					player.getInventory().delete(14986, 1);
					player.getInventory().delete(14983, 1);
					player.getInventory().delete(14984, 1);
					player.getInventory().delete(14985, 1);
					player.getInventory().add(14980, 1);
					player.sendMessage("@red@You managed to save Gorgie before it was too late!");
					player.getPacketSender().sendInterfaceRemoval();
				}
				break;
			case 2506:
				if ((player.getInventory().getAmount(5022))  < 1000)  {
					player.sendMessage("@red@You need 1000 Tickets to offer to Pennywise");
					player.getPacketSender().sendInterfaceRemoval();
				return;
				}
				if ((player.getInventory().getAmount(5022))  > 1000)  {
					player.getInventory().delete(5022, 1000);
					player.performAnimation(new Animation(827));
					int pennywise = RandomUtility.exclusiveRandom(25);
					if (pennywise == 9 ) {
					player.getInventory().add(14986, 1);
					player.sendMessage("@red@Pennywise hands over a random Georgie piece.");
					player.getPacketSender().sendInterfaceRemoval();
					}
					int pennywise1 = RandomUtility.exclusiveRandom(25);
					if (pennywise1 == 8 ) {
					player.getInventory().add(14983, 1);
					player.sendMessage("@red@Pennywise hands over a random Georgie piece.");
					player.getPacketSender().sendInterfaceRemoval();
					}
					int pennywise2 = RandomUtility.exclusiveRandom(25);
					if (pennywise2 == 7 ) {
					player.getInventory().add(14984, 1);
					player.sendMessage("@red@Pennywise hands over a random Georgie piece.");
					player.getPacketSender().sendInterfaceRemoval();
					}
					int pennywise3 = RandomUtility.exclusiveRandom(25);
					if (pennywise3 == 6 ) {
					player.getInventory().add(14985, 1);
					player.sendMessage("@red@Pennywise hands over a random Georgie piece.");
					player.getPacketSender().sendInterfaceRemoval();
					}
					
					else player.sendMessage("@red@Pennywise takes your tickets, and in return doesn't kill you.");
					player.getPacketSender().sendInterfaceRemoval();
				}
				break;
			...
			case 99: //shrine mg
				 if (player.getInventory().getAmount(3706)  < 250) {
					...
					player.getInventory().delete(3706, 250);
					player.getInventory().add(5125, 1);
					break;
			
			case 1111: //elder boss 3 option
				if (player.getInventory().getAmount(10835)  < 10000000) {
			...
			case 102:// yellow stone take
				if (player.getInventory().getAmount(15286)  < 12000) {
				...
				player.getInventory().delete(15286, 12000);
				player.getInventory().add(6641, 1);
				break;
			
			case 1020:// yellow stone take
				if (player.getInventory().getAmount(6641)  < 1) {
					
					player.getPacketSender()
				.sendMessage("@blu@You need at least 1 @red@yellow crystal"
				+ "@red@To get the aura");
				return;
				}
				player.getInventory().delete(6641, 1);
				player.getInventory().add(3307, 1);
				break;
			...
			case 5:
				...
				if (player.getRights() == PlayerRights.PLAYER) {
					player.getPacketSender().sendMessage("You need to be a member to teleport to this zone.")
							.sendMessage("To become a member, visit VortexPS/store/ and purchase a scroll.");
					return;
				}
			...
			case 743:
				if ((GameSettings.DOUBLE_EXP == true) || (GameSettings.TRIPLE_EXP == true)) {
					player.sendMessage("@blu@The Custom Well Of Goodwill is currently full");
					player.getPacketSender().sendInterfaceRemoval();
				} else if (player.getInventory().contains(10835, 20000)) {
					player.getInventory().delete(10835, 20000);
					player.incrementCustomWellDonated(1);
					GameSettings.DOUBLE_EXP = true;
					World.sendMessage("<img=10>@blu@" + player.getUsername().toString()
							+ " @bla@Has just donated @red@20K bags@bla@ for @red@4h Double Exp (serverwide)");
					Timer t = new Timer();
					t.schedule(new TimerTask() {
						@Override
						public void run() {
					...
					player.sendMessage("@blu@You don't have 20K Pwnlite Taxbags in ur inv :(");
					player.getPacketSender().sendInterfaceRemoval();
				}
				break;
			case 9:
			...
			case 2503://yhitd option
			TeleportHandler.teleportPlayer(player, new Position(3182, 9648),
					player.getSpellbook().getTeleportType());
			...
			case 31:
				...
				break;
			case 45:
				player.getPacketSender().sendString(1, "VortexPS.com");
				break;
			}
		} else if (id == FIRST_OPTION_OF_TWO) {
			switch (player.getDialogueActionId()) {
			case 12000:
				if (player.payAoePayment()) {
					player.getAoEInstance().start();
			...
			case 33:
				...
				break;
			case 35:
				if (player.getPointsHandler().getBloodSlayerPoints() <= 25) {
					player.getPacketSender().sendMessage("You need at least 25 points to reset your task!");
					return;
				} else {
					player.getPacketSender().sendInterfaceRemoval();
					//player.getSlayer().resetSlayerTask1();
				}
				break;
			case 34:
				player.getPacketSender().sendInterfaceRemoval();
			...
			case 633:
				...
				player.getPacketSender().sendEnterInputPrompt("How many Vote scrolls would u like to donate?");
				break;
			...
			case 633:
				player.sendMessage("I can take@red@ " + BlackstoneWarmonger.amountNeeded + " @bla@more Vote scrolls!");
				player.getPacketSender().sendInterfaceRemoval();
				break;
			...
			case 5:
				...
				if (player.getRights() == PlayerRights.PLAYER) {
					player.getPacketSender().sendMessage("You need to be a member to teleport to this zone.")
							.sendMessage("To become a member, visit VortexPS.com/store/ and purchase a scroll.");
					return;
				}
				TeleportHandler.teleportPlayer(player, new Position(3363, 9638),
						player.getSpellbook().getTeleportType());
			...
			case 71:
				...
				if (player.getInventory().contains(19670) && player.getClickDelay().elapsed(700)) {
					...
					int pointAmount = player.getRights().isDeluxeDonator(player) ? 2 : 1;
					player.getInventory().delete(19670, amt);
					player.getPointsHandler().incrementVotingPoints(amt * pointAmount * 3);
					player.getPointsHandler().refreshPanel();
					player.getInventory().add(10835, 2500 * amt);
					player.getPacketSender().sendMessage("You claim the " + (amt > 1 ? "scrolls" : "scroll")
							+ " and receive your reward including " + pointAmount * 3 + " Vote Points");
					StarterTasks.finishTask(player, StarterTaskData.REDEEM_A_VOTE_SCROLL);
					int chance = RandomUtility.exclusiveRandom(500);
					if (chance >= 498 ) {
						player.getInventory().add(3824, 1);
						World.sendMessage("<col=fcfcfc><shad=dd8a14><img=418>[Voting]</shad>@bla@: " + player.getUsername().toString()
								+ " Has just received a @red@Vote Mystery Box!");
					}
					if (chance >= 499) {
						player.getInventory().add(10168, 1);
						World.sendMessage("<col=fcfcfc><shad=dd8a14><img=418>[Voting]</shad>@bla@: " + player.getUsername().toString()
								+ " Has just received a @red@Owners box from voting!");
					}
					player.getClickDelay().reset();
				}
				break;
