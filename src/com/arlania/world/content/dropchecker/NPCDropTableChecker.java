...
public class NPCDropTableChecker {
	...
	public void getActionIdForName(Player player, int npcId) {
		try {
			...
			for (int i = 0; i < dropTableNpcNames.size(); i++) {
				if (name.equalsIgnoreCase(dropTableNpcNames.get(i))) {
			...
			player.sendMessage("This NPC has no drops to view!");// try
		} catch (Exception e) {
			player.sendMessage("This NPC has no drops to view!");
		}
	}
	...
	public void showNPCDropTable(final Player player, final int npcId) {
		...
		for (Entry<DropChance, List<Item>> entry : dropTables.entrySet()) {
			...
			} else if (chance == DropChance.LEGENDARY_6) {
				player.getPacketSender().sendItemsOnInterface(37924, 60, dropTable, false);
			} else if (chance == DropChance.LEGENDARY_7) {
					player.getPacketSender().sendItemsOnInterface(37925, 60, dropTable, false);
			} else if (chance == DropChance.LEGENDARY_8) {
					player.getPacketSender().sendItemsOnInterface(37926, 60, dropTable, false);
			} else if (chance == DropChance.LEGENDARY_9) {
				player.getPacketSender().sendItemsOnInterface(37927, 60, dropTable, false);
			} else if (chance == DropChance.LEGENDARY_10) {
			player.getPacketSender().sendItemsOnInterface(37928, 60, dropTable, false);
			}
