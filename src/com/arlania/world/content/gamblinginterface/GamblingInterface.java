...
public class GamblingInterface {
	...
	public void requestGamble(Player player2) {
		...
		if(player.getGameMode() == GameMode.GROUP_IRONMAN) {
			player.getPacketSender().sendMessage("@red@You are an Iron Man. You stand alone.");
			return;
		}
 
		if(player2.getGameMode() == GameMode.GROUP_IRONMAN) {
			player.getPacketSender().sendMessage(player2.getUsername() + " @red@is an Iron Man. He stands alone.");
			return;
		}
		
		if (player.getGameMode() == GameMode.IRONMAN) {
			player.getPacketSender().sendMessage("@red@Ironman-players are not allowed to gamble.");
			return;
		}
		if (player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
			player.getPacketSender().sendMessage("@red@Hardcore-ironman-players are not allowed to gamble.");
			return;
		}
		if (player2.getGameMode() == GameMode.IRONMAN) {
			player.getPacketSender()
					.sendMessage("@red@That player is a Hardcore-ironman-player and can therefore not gamble.");
			return;
		}
		if (player2.getGameMode() == GameMode.HARDCORE_IRONMAN) {
			player.getPacketSender().sendMessage("@red@That player is an Ironman player and can therefore not gamble.");
			return;
		}
		if (player.getLocation() == Location.DUNGEONEERING) {
			player.getPacketSender().sendMessage("@red@You are far too busy to gamble at the moment!");
			return;
		}
		...
		if (System.currentTimeMillis() - lastGambleSent < 5000 && !inGamble()) {
			player.getPacketSender().sendMessage("@red@You're sending gamble requests too frequently. Please slow down.");
			return;
		}
		...
		if (player.getLocation() == Location.GODWARS_DUNGEON
			...
			player.getPacketSender().sendMessage("@red@You cannot reach that.");
			return;
		}
		...
		if (player2.busy() || player2.getInterfaceId() > 0 || player2.getGambling().inGamble() || player2.isBanking()
			...
			player.getPacketSender().sendMessage("@red@The other player is currently busy.");
			return;
		}
		if (player.getInterfaceId() > 0 || inGamble() || player.isBanking()
			...
			player.getPacketSender().sendMessage("@red@You are currently unable to gamble another player.");
			if (player.getInterfaceId() > 0)
				player.getPacketSender()
						.sendMessage("@red@Please close all open interfaces before requesting to open another one.");
			return;
		}
		...
		if (!Locations.goodDistance(player.getPosition().getX(), player.getPosition().getY(),
			...
			player.getPacketSender().sendMessage("@red@Please get closer to request a gamble.");
			return;
		}
		if (!inGamble() && player2.getGambling().gambleRequested()
			...
			player.getPacketSender().sendMessage("You've sent a gamble request to @red@" + player2.getUsername() + ".");
	...
	public void declineGamble(boolean tellOther) {
		...
		if (tellOther && getGambleWith() > -1) {
			...
			player2.getPacketSender().sendMessage("@red@Other player declined the gamble.");
		}
		resetGamble();
	}
	...
	public void gambleItem(int itemId, int amount, int slot) {
		...
		if (player.getRights() != PlayerRights.DEVELOPER && player2.getRights() != PlayerRights.DEVELOPER
			...
			if (!new Item(itemId).tradeable()) {
				player.getPacketSender().sendMessage("This item is currently cannot be gambled.");
				return;
			}
		}
		falseGambleConfirm();
	...
	public void acceptGamble(int stage) {
		...
		if (stage == 1) {
			...
			player.getPacketSender().sendString(57195, "Blackjack (@red@" + player.getUsername() + "@bla@ hosts)");
			player2.getPacketSender().sendString(57195, "Blackjack (@red@" + player.getUsername() + "@bla@ hosts)");
			player.getPacketSender().sendString(57197, "55x2 (@red@" + player.getUsername() + "@bla@ hosts)");
			player2.getPacketSender().sendString(57197, "55x2 (@red@" + player.getUsername() + "@bla@ hosts)");
			if (inGamble() && player2.getGambling().gambleConfirmed && player2.getGambling().goodGamble && goodGamble) {
				startGame(getGamblingMode());
				System.out.println("@red@Confirmed");
			}
		}
	...
	private void startDiceDuel() {
		...
		//player.moveTo(new Position(2743, 3645));
		//player2.moveTo(new Position(2743, 3647));
		player.setDirection(Direction.EAST);
		player2.setDirection(Direction.WEST);
		...
		TaskManager.submit(new Task(4) {
			...
			protected void execute() {
				...
				player.forceChat("I rolled a : " + randomRoll + " - Score: " + ddScore);
				player2.forceChat("I rolled a : " + randomRoll1 + " - Score: " + player2.getGambling().ddScore);
	...
	public boolean gambleFinished = true;
	...
	private void sendDescriptionForMode(GamblingMode mode) {
		...
		if (mode.equals(GamblingMode.DICE_DUEL)) {
			player.getPacketSender().sendString(57157, "@or2@The @or1@higher roll @or2@gets the point")
					.sendString(57158, "@or2@First to @red@3 @or2@points wins.")
			.sendString(57159, "");
			
		} else if (mode.equals(GamblingMode.FLOWER_POKER)) {
			player.getPacketSender().sendString(57157, "@or2@Each player plants 5 flowers")
					.sendString(57158, "@or2@Whoever gets @or1@more pairs @or2@wins")
					.sendString(57159, "");
		} else if (mode.equals(GamblingMode.BLACKJACK)) {
			player.getPacketSender().sendString(57157, "@or2@Roll dice, Hit or stay")
					.sendString(57158, "@or2@Closest to @or1@100 Wins. @or2@Go over you loose.")
					.sendString(57159, "");
		} else if (mode.equals(GamblingMode.FIFTY_FIVE_X2)) {
			player.getPacketSender().sendString(57157, "@or2@If host rolls @or1@over 55@or2@ the player wins.")
					.sendString(57158, "@or2@Otherwise the host wins.")
					.sendString(57159, "");
		}
	...
	Position[][] flowerPokerLocations = new Position[][] {
		new Position[] { new Position(3114, 2900, 0), new Position(3114, 2899, 0) },
		new Position[] { new Position(3103, 2899, 0), new Position(3103, 2900, 0) },
		new Position[] { new Position(3092, 2900, 0), new Position(3092, 2899, 0) },
		
		new Position[] { new Position(3092, 2919, 0), new Position(3092, 2918, 0) },
		new Position[] { new Position(3103, 2919, 0), new Position(3103, 2918, 0) },
		new Position[] { new Position(3114, 2919, 0), new Position(3114, 2918, 0) },
	...
	private void startFlowerPoker() {
		...
		int randomLocation = RandomUtility.inclusiveRandom(0, flowerPokerLocations.length - 1);
		
		player.moveTo(flowerPokerLocations[randomLocation][0]);
		player2.moveTo(flowerPokerLocations[randomLocation][1]);
	...
	private void plantFlower() {
		...
		//MovementQueue.stepAway(player);
		player2.getMovementQueue().walkStep(-1, 0);
	...
	private void startBlackjack() {
		...
		//player.moveTo(new Position(2732, 3469));
		//player2.moveTo(new Position(2732, 3468));
	...
	private void getFPWinner() {
		...
		if (result1 == result2) {
			...
			TaskManager.submit(new Task(4) {
				@Override
				protected void execute() {
					player2.forceChat("I win with " + result2);
					for(GameObject object : plants) {
						System.out.println("-executed-");
						CustomObjects.globalObjectRemovalTask(object, 1);
					}
	...
	private void resetData() {
		...
		player.getPacketSender().sendString(57156, "@red@Rules for" + rules);
		player.getPacketSender().sendString(57157, "").sendString(57158, "").sendString(57159, "");
	}
