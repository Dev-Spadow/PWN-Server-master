...
public final class CharacterList<E extends Character> implements Iterable<E> {
    ...
    private int expandOnDemand() { // expand the list on demand by some amount, this should only be called when the previous list is full instead of every time, altho for now this isn't necessary
        CharacterList<NPC> current = World.getNpcs();
        int curSize = current.capacity();
        CharacterList<NPC> updated = new CharacterList<>(current.capacity() + 256);
    ...
    public boolean remove(E e) {
        ...
        if (e.isPlayer()) {
            ...
            if (player.getSession().getState() != SessionState.LOGGING_OUT && !player.isMiniPlayer()) {
				player.dispose();
				return false;
			}
        }
