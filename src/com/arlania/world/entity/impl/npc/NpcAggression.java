...
public final class NpcAggression {
	...
	public static void target(Player player) {
		...
		for (NPC npc : player.getLocalNpcs()) {
			...
			if (npc == null || npc.getConstitution() <= 0 || !(dung && npc.getId() != 11226)
					&& !npc.getDefinition().isAggressive() && player.agroPotionTime == LocalDateTime.MIN) {
				continue;
			}
			...
			if (Locations.goodDistance(npc.getPosition(), player.getPosition(), npc.getAggressionDistance())
				...
				if (player.getSkillManager().getCombatLevel() > (npc.getDefinition().getCombatLevel() * 2)
						&& player.getLocation() != Location.WILDERNESS && !dung && !npc.getDefinition().isAggressive()
				...
				if (Location.ignoreFollowDistance(npc) || gwdMob
						...
						|| dung) {
					if (CombatFactory.checkHook(npc, player) && npc.getDefinition().isAttackable()) {
