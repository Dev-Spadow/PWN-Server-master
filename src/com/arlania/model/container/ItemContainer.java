...
public abstract class ItemContainer {
	...
	public boolean containsWithAmount2(int id, int amount) {
		for (Item items : this.items) {
			if (items != null && items.getId() == id && items.getAmount() >= amount) {
				return true;
			}
	...
	public boolean containsWithAmount(Item[] item) {
		if (item.length == 0) {
			return false;
		}
		...
		for (Item nextItem : item) {
			...
			if (!containsWithAmount2(nextItem.getId(), nextItem.getAmount())) {
				return false;
			}
		}
		return true;
			...
			if (slot == -1) {
				if (getPlayer().getRights() != PlayerRights.ADMINISTRATOR
						&& getPlayer().getRights() != PlayerRights.OWNER
								&& getPlayer().getRights() != PlayerRights.CO_OWNER
	...
	public ItemContainer delete(Player player, int id, int slot, int amount) {
		System.out.println("x1");
		if (id <= 0 || slot < 0) {
			return this;
		}
		int xxx = player.getInventory().getItems()[slot].getId();
		System.out.println("x2 "+xxx+" id "+id);
		if (player.getInventory().getItems()[slot].getId() == (id)) {
			System.out.println("x3 "+player.getInventory().getItems()[slot].getAmount()+" amount "+amount);
			if (player.getInventory().getItems()[slot].getAmount() < amount) {
				System.out.println("x5");
				player.getInventory().getItems()[slot].setAmount(player.getInventory().getItems()[slot].getAmount()-amount);
			} else {
				System.out.println("x4");
				player.getInventory().getItems()[slot].setAmount(0);
				player.getInventory().getItems()[slot].setId(-1);
			}
			refreshItems();
		}
		return this;
