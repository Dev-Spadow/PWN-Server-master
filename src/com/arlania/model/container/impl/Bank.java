...
public class Bank extends ItemContainer {
	...
	public Bank open(boolean openGroup) {
		if(getPlayer().getGameMode().equals(GameMode.GROUP_IRONMAN) && openGroup) {
			GamemodeSelecter.sendOptionsTitle(getPlayer(), "Which of the following storages would you like to open?", "Personal Storage", "Group Storage", true);
			getPlayer().setActionInterface("groupStorage_open");
			return null;
		}
	...
	public static class BankSearchAttributes {
		...
		public static void beginSearch(Player player, String searchSyntax) {
			...
			player.getBankSearchingAttribtues().getSearchedBank().open(false);
		...
		public static void stopSearch(Player player, boolean openBank) {
			...
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0)
					.sendString(5383, "        The Bank of Pwnlite");
			if (openBank)
