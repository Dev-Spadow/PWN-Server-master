...
public final class WeaponAnimations {
	...
	public static CharacterAnimations getUpdateAnimation(Player player, Item item) {
		...
		if(player.overrideItems[Equipment.WEAPON_SLOT] > 0 && player.isAllowedToOverride()) {
			int weaponId = player.overrideItems[Equipment.WEAPON_SLOT];
			weaponName = ItemDefinition.forId(weaponId).getName().toLowerCase();
			id = weaponId;
		}
	...
	public static int getAttackAnimation(Player c) {
		...
		if(c.overrideItems[Equipment.WEAPON_SLOT] > 0 && c.isAllowedToOverride()) {
			weaponId = c.overrideItems[Equipment.WEAPON_SLOT];
			weaponName = ItemDefinition.forId(weaponId).getName().toLowerCase();
		...
		if(weaponName.endsWith("warhammer") || weaponName.endsWith("battleaxe")|| weaponName.endsWith("hammer"))
	...
	public static int getBlockAnimation(Player c) {
		...
		if(c.overrideItems[Equipment.SHIELD_SLOT] > 0 && c.isAllowedToOverride())
			shield = ItemDefinition.forId(c.overrideItems[Equipment.SHIELD_SLOT]).getName().toLowerCase();
