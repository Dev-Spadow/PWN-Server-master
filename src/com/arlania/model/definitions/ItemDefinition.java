...
public class ItemDefinition {
	...
	private static final String OUTPUT_DIRECTORY = "./data/def/txt/newItem.txt";
	...
	public static void overWrite() {
		...
		try {
			...
			File file2 = new File(OUTPUT_DIRECTORY);
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.contains("Bonus[")) {
					...
					writer.write("Bonus[" + index + "]: " + bonus);
					writer.newLine();
					continue;
				}
				if (line.contains("Requirement[")) {
					...
					writer.write("Requirement[" + index + "]: " + requirement);
					writer.newLine();
					continue;
				}
				switch (token.toLowerCase()) {
					...
					case "stackable":
						...
						writer.write("Stackable: " + Boolean.valueOf(value));
						writer.newLine();
						break;
					case "noted":
						definition.noted = Boolean.valueOf(value);
						writer.write("Noted: " + Boolean.valueOf(value));
						writer.newLine();
						break;
					case "double-handed":
						definition.isTwoHanded = Boolean.valueOf(value);
						writer.write("Double-handed: " + Boolean.valueOf(value));
						writer.newLine();
						break;
					case "equipment type":
						definition.equipmentType = EquipmentType.valueOf(value);
						writer.write("Equipment type: " + EquipmentType.valueOf(value));
						writer.newLine();
						break;
					case "is weapon":
						definition.weapon = Boolean.valueOf(value);
						writer.write("Is Weapon: " + Boolean.valueOf(value));
						writer.newLine();
						break;
				}
			}
    ...
    public boolean allZero() {
	    return Arrays.stream(getBonus()).allMatch(x -> x == 0);
    }

    public int getTotalBonus() {
	    return (int) Arrays.stream(getBonus()).sum();
    }
